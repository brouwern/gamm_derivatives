...
I use Generalized Addidive Model (GAM; Wood 2006) to model how PAR decreases with time. This could be fanicer by adding autocorrelation (eg AR1) to the errors but I don't think that accomplished anything really.

Also, instead of means for 10 AM to 2 PM we could directly model the raw data with random effects with a GAMM (Generalized Additive Mixed Model), but that wouldn't add anything to the anlysis.



### Run the GAM

The notation s(julian) tells the model to build a smoother for the covariation "julian".
```{r}
...
Function is called "Deriv"
```{r}
#https://gist.github.com/gavinsimpson/e73f011fdaaab4bb5a30
source("./R/Deriv.R")
```

The derivative function has the key arguements (with their defaults):
